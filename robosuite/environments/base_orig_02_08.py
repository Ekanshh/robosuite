...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def _post_action(self, action):
        ...
        self.done = ((self.timestep >= self.horizon) or
                     (self._check_success() and self.num_via_point == 1 and self.success == 2) and not self.ignore_done)
    ...
    def built_min_jerk_traj(self):
        ...
        return:
        '''
        global pos_in, ori_in
        pos = self.via_point['p' + str(self.num_via_point)][:3]
        ori = self.via_point['p' + str(self.num_via_point)][3:]

        if self.num_via_point == 0:
            pos_in = self.pos_in[0]
            ori_in = self.pos_in[1]
            ori_in[0] = abs(ori_in[0])
            # pos_in[2] = 0.95
        else:
            if self.num_via_point == 1 and self.enter == 1:
        ...
        if self.num_via_point < (self.num_via_point-1):
            time_to = self.loop_time * self.trans
        else:
            time_to = self.loop_time * (1 - self.trans) * 0.5
            # time_to = self.loop_time * (1 - sum(self.trans[str(x)] for x in range(self.num_via_point - 1)))  # * 0.1
    ...
    def built_next_desired_point(self):
        if self.num_via_point == 1 and self.enter == 1:
            self.t_bias = self.sim.data.time - 0.002
            self.enter = 0
        t_now = self.sim.data.time - self.t_bias
        ...
        # right_omega_desired = self.sim.data.get_body_xmat("robot0_right_hand").T @ right_omega_desired
    ...
    def calc_next_desired_point(self):
        ...
        if self.num_via_point < 1:
            if self.timestep == round((self.horizon * self.trans)):
