...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    def __init__(self, check_freq: int, log_dir: str, verbose=1):
        ...
        self.save_path = os.path.join(log_dir, 'best_model_12_05s')
...
if __name__ == "__main__":
    ...
    control_param = dict(type='IMPEDANCE_PB_Partial', input_max=1, input_min=-1, output_max=[0.05, 0.05, 0.05, 0.5, 0.5, 0.5],
                         output_min=[-0.05, -0.05, -0.05, -0.5, -0.5, -0.5], kp=50, damping_ratio=1/np.sqrt(2),
                         impedance_mode='fixed', kp_limits=[0, 100000], damping_ratio_limits=[0, 10], position_limits=None,
                         orientation_limits=None, uncouple_pos_ori=True, control_delta=True, interpolation=None,
                         ramp_ratio=0.2, control_dim=108)
    ...
    # model = PPO.load("best_model_27_04", verbose=1)
    # model.set_env(env)
    # model.learn(total_timesteps=8000, tb_log_name="big_board", callback=reward_callback)#, eval_env=evaluate(model, env, n_eval_episodes=10))
    # print("Done")
    # model.save(str("big_board_5_cm"))

    # plot_results([log_dir], num_timesteps=100 * 500, x_axis="timesteps", task_name="Rewards")

    mean_reward, std_reward, episode_success = evaluate(model, env, n_eval_episodes=50)
